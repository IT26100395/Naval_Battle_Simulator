...
double calculateHitDetails(double distance, double velocity, double minAngleLimit, double *outAngleDeg) {
    ...
    if (angleDeg < minAngleLimit) {
        double highAngleDeg = 90.0 - angleDeg; // Alternate high trajectory
...
/* Generate Escort Ships randomly based on Table 1 Specification */
...
/* Custom Target Strategy: Sort in-range Escorts by highest Impact Power */
void sortTargetsByPriority(int targetIndices[], int count, EscortShip escorts[], Battleship b) {
...
int main() {
    ...
    EscortShip escortsCopy[MAX_ESCORTS];
    Point path[MAX_PATH_POINTS];

    printf("====================================================\n");
    printf("     NAVAL BATTLE SIMULATOR - PART 2-A COMPLETE     \n");
    printf("====================================================\n\n");
    ...
    memcpy(escortsCopy, escorts, sizeof(escorts)); // Backup for multi-phase simulation

    /* Save Initial Conditions to Text File (Part 1-A Output) */
    FILE *initFile = fopen("part2a_initial.txt", "w");
    if (initFile) {
        fprintf(initFile, "--- INITIAL BATTLEFIELD CONDITIONS (PART 1-A & PART 2-A) ---\n");
        ...
        fprintf(initFile, "Escort Ships Data:\n");
        for (int i = 0; i < numberOfEscorts; i++) {
            fprintf(initFile, "E%d [%s] Pos: (%.2f, %.2f) | Vmin: %.2f | Vmax: %.2f | MinAngle: %.2f | MaxAngle: %.2f | Impact: %.2f\n",
                    escorts[i].id, escorts[i].type, escorts[i].x, escorts[i].y,
                    escorts[i].minVelocity, escorts[i].maxVelocity, escorts[i].minAngle, escorts[i].maxAngle, escorts[i].impactPower);
        }
        fclose(initFile);
    }

    FILE *resFile = fopen("part2a_results.txt", "w");

    /* ==================================================== */
    /* STAGE 1: PART 1-A REDO (Static Snapshot Analysis)   */
    /* ==================================================== */
    printf("\n====================================================\n");
    printf("      STAGE 1: PART 1-A REDO (STATIC ANALYSIS)      \n");
    printf("====================================================\n");
    if (resFile) fprintf(resFile, "=== STAGE 1: PART 1-A REDO (STATIC ANALYSIS) ===\n");

    printf("Static Assessment at Initial Position (%.2f, %.2f):\n", b.x, b.y);
    for (int i = 0; i < numberOfEscorts; i++) {
        double dist = calculateDistance(b.x, b.y, escorts[i].x, escorts[i].y);
        double minR, maxR;
        escortAttackRange(escorts[i], &minR, &maxR);
        int canHitB = (dist >= minR && dist <= maxR);

        double bMaxRange = (b.maxVelocity * b.maxVelocity) / G;
        int bCanHit = (dist <= bMaxRange);

        printf(" - E%d (%s) Dist: %6.2fm | Escort Can Hit B: %s | B Can Hit Escort: %s\n",
               escorts[i].id, escorts[i].type, dist, canHitB ? "YES" : "NO ", bCanHit ? "YES" : "NO ");
        if (resFile) {
            fprintf(resFile, " - E%d (%s) Dist: %6.2fm | Escort Can Hit B: %s | B Can Hit Escort: %s\n",
                    escorts[i].id, escorts[i].type, dist, canHitB ? "YES" : "NO ", bCanHit ? "YES" : "NO ");
        }
    }

    /* ==================================================== */
    /* STAGE 2: PART 2-A / PART 1-C FULL DYNAMIC SIMULATION */
    /* ==================================================== */
    printf("\n====================================================\n");
    printf("   STAGE 2: PART 2-A FULL DYNAMIC SIMULATION        \n");
    printf("====================================================\n\n");
    if (resFile) fprintf(resFile, "\n=== STAGE 2: PART 2-A FULL DYNAMIC SIMULATION ===\n\n");

    /* Global Continuous Time Clock across all steps */
    ...
    for (int step = 1; step <= kPoints; step++) {
        ...
        /* 1. Escorts Attack Battleship First */
        for (int i = 0; i < numberOfEscorts; i++) {
        ...
        /* 2. Battleship Counter-Attacks using Custom Strategy & Time Gap (T_B) */
        if (b.cumulativeImpact < 1.0) {
            double bMaxRange = (b.maxVelocity * b.maxVelocity) / G;
            ...
            /* Apply Custom Strategy: Highest Impact Power First */
            sortTargetsByPriority(validTargets, validCount, escorts, b);

            /* Print Attack Order */
            printf("\n--> Battleship Attack Order (Strategy: Highest Impact First):\n[ ");
            if (resFile) fprintf(resFile, "\n--> Battleship Attack Order (Strategy: Highest Impact First):\n[ ");
            ...
            /* Execute Firing Sequence considering T_B Delay */
            for (int k = 0; k < validCount; k++) {
                ...
                if (flightTime > 0) {
                    ...
                    currentSimTime += b.firingDelay; // Accumulates continuously
    ...
    printf("\nOutput files updated successfully: 'part2a_initial.txt' & 'part2a_results.txt'\n");
    return 0;
}
